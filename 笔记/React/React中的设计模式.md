...
与性能优化的视角相似，当我们谈论React中的设计模式的时候，实际上是在谈论“React组件的设计模式”。

在React设计模式中，最重要，也是最为人津津乐道的就是下面这三个模式：

- 高阶组件（HOC）
- render Props
- 剥离有状态的组件和无状态的组件

除此之外，每个团队可能都有自己的设计模式方法论。这些形态各异的React设计模式虽然实现思路有所不同，但是其本质都是为了实现这样一个终极目标——

**以尽可能优雅的姿态，实现组件的逻辑复用**。

首先要做的一件事，就是把每一种设计模式到底**是什么**，**怎么用**弄清楚。在这个过程中，体会**单一职责**和**开放封闭**这两个非常重要的设计原则。
...
高阶组件（Higher Order Components）在概念上沿袭了高阶函数（Higher-Order Function）。我们已经和高阶函数打过交道，这里我们再复习一下高阶函数的概念：**接收函数作为输入，或者输出另一个函数的一类函数，就是高阶函数**(换句话说：函数作为参数，或者函数作为值被返回)。
